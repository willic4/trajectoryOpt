...
close all;
clear;
global initialstate gridN finalstate im idx fig
idx = 0;
%% trajectory options
...
% J = @(x) w1*((finalstate(1)-x(gridN+1))^2 + (finalstate(2)-x(2*gridN+1))^2 + (finalstate(3)-x(3*gridN+1))^2 + (finalstate(4)-x(4*gridN+1))^2)+w2*(x(1)); %cost function - designed to minimize insertion error
J = @(x) w1*(sum(abs(x(2 + gridN * 4 : 1 + gridN * 5))))+ w2*(sum(abs(x(2 + gridN * 5 : 1 + gridN * 6))))... 
    +w3*(x(1)); %cost function minimize total accelerations
...
Aeq=[];
beq=[];
lb = [1;    ones(gridN * 1, 1) * -Inf; ones(gridN * 1, 1) * 0; 0 ; ones(gridN-1, 1) * -Inf; 0 ; ones(gridN-1, 1) * -Inf; ones(2*gridN, 1) * -maxacc];
ub = [Inf;  ones(gridN * 1, 1) * Inf ; ones(gridN * 1, 1) * finalstate(2); 50; ones(gridN-1, 1) *  Inf; 50; ones(gridN-1, 1) *  Inf; ones(2*gridN, 1) *  maxacc];
...
global gridN initialstate finalstate
    c=[];
