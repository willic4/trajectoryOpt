...
initialstate = [0, r_earth+1000e3, sqrt(mu/(r_earth+1000e3)), 0]; 
...
time_min = @(x) abs(x(gridN+2) - x(2*gridN+1));
...
x0 = [1; [(r_earth+1000e3)*sin(0:2*pi/50:2*pi-2*pi/50)]'; [(r_earth+1000e3)*cos(0:2*pi/50:2*pi-2*pi/50)]'; [sqrt(mu/(r_earth+1000e3))*cos(0:2*pi/50:2*pi-2*pi/50)]'; [sqrt(mu/(r_earth+1000e3))*sin(0:2*pi/50:2*pi-2*pi/50)]'];
x0 = [1; [(r_earth+1000e3)*sin(0:2*pi/50:2*pi-2*pi/50)]'; [(r_earth+1000e3)*cos(0:2*pi/50:2*pi-2*pi/50)]'; linspace(1,1,gridN)'; linspace(1,1,gridN)'];

x0 = [1; [(r_earth+1000e3)*sin(0:pi/(gridN):pi - pi/(gridN))]'; [(r_earth+1000e3)*cos(0:pi/(gridN):pi - pi/(gridN))]'; [sqrt(mu/(r_earth+1000e3))*cos(0:pi/(gridN):pi - pi/(gridN))]'; [-sqrt(mu/(r_earth+1000e3))*sin(0:pi/(gridN):pi - pi/(gridN))]'];

% x0 = [1; [(r_earth+1000e3)*sin(0:pi/(gridN-1):pi)]'; [(r_earth+1000e3)*cos(0:pi/(gridN-1):pi)]'; [sqrt(mu/(r_earth+1000e3))*cos(0:pi/(gridN-1):pi)]'; [-sqrt(mu/(r_earth+1000e3))*sin(0:pi/(gridN-1):pi)]'];
% x0 = [1; linspace(1,1,gridN)'; linspace(1,1,gridN)'; linspace(1,1,gridN)'; linspace(1,1,gridN)']
...
% Aeq = [0,1,0,0,0,zeros(1,(gridN-1)*4);... 
%        0,0,1,0,0,zeros(1,(gridN-1)*4);... 
%        0,0,0,1,0,zeros(1,(gridN-1)*4);... 
%        0,0,1,0,1,zeros(1,(gridN-1)*4)];   
...
lb = [0;    ones(gridN * 4, 1) * -Inf];
ub = [Inf;  ones(gridN * 4, 1) * Inf];
% Options for fmincon
...
global gridN maxacc initialposition initialvelocity w impactpos Thrust mu a_desired initialstate
    c=[];
...
%     ceq = [initialposition(1) - xs(1); initialposition(2) - ys(1);  initialposition(3) - zs(1); initialvelocity(1) - xds(1); initialvelocity(2) - yds(1); initialvelocity(3) - zds(1); alphas(1); phis(1)];
    ceq=[xs(1) - initialstate(1); ys(1) - initialstate(2);xds(1) - initialstate(3); yds(1) - initialstate(4)];
    for i = 1 : length(xs) - 1
        % The state at the beginning of the time interval
        x_i = [xs(i); ys(i); xds(i); yds(i)];
        r_i = [xs(i);ys(i)];
        gravity_i = -mu/norm(r_i)^3 * r_i; % constrained 2BP
        xdot_i = [xds(i); yds(i); gravity_i(1); gravity_i(2)];
        ...
        xdot_n = [xds(i+1); yds(i+1); gravity_n(1); gravity_n(2)];
...
%         xend = STM * x_i;
    ...
    ceq = [ceq ; xs(end) + initialstate(1); ys(end) + initialstate(2);xds(end) + initialstate(3); yds(end) + initialstate(4)];
end
