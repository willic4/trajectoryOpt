...
outFolderName = "C:\Users\billy\Documents\GitHub\trajectoryOpt\Sandbox\Presentationworthy\excursion_realmass\";   % Your destination folder
...
vehiclemass = 152905; %mass in kg % will give 10 g max acc
...
global gridN initialstate finalstate finalr mu r_earth
    ...
    ceq=[initialstate(1) - xs(1); initialstate(2) - ys(1); initialstate(3) - xds(1); initialstate(4) - yds(1)];%; pi/2 - ycmd];
    for i = 1 : length(xs) - 1
        c = [c; abs(ycmd(i+1) - ycmd(i)) - 10*pi/180];
        v_i = [xds(i);yds(i)];
        c = [c;  abs(atan2(v_i(2),v_i(1))-ycmd(i)) - 10*pi/180]; %limit angle of attack to 10 degrees
...
if strcmp(state,'iter')
    ...
    ylabel('Y (km)')
    subplot(2,2,3:4)
    plot(times,xcmd*1000);
    grid on
    hold on
    plot(times,ycmd*180/pi);
    title('Commands')
    legend('Axial Acceleration (m/s^2)','alpha (deg)')
    ylim([-180,500])
    xlabel('time (s)')
    hold off
    drawnow
